...
nl80211_get_wiphy_data_ap(struct i802_bss *bss)
{
    ...
    //TODO: check this logic and see if needed or not
                               ...
                               int len, int next_avail_data_not_used)
{
    ...
    if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_SIGNAL_VALID)
        sig_change->current_signal = info->sta_info.signal;
    ...
    if (info->sta_info.valid_fields & NRF_WIFI_STA_INFO_TX_BITRATE_VALID) {
        if (info->sta_info.tx_bitrate.valid_fields & NRF_WIFI_RATE_INFO_BITRATE_VALID)
            sig_change->current_txrate = info->sta_info.tx_bitrate.bitrate * 100;
    }
	 _arg->result_finished = 0;
    return NL_SKIP;
}
...
#ifdef CFG_INTERFACE
    // auth_info->nrf_wifi_flags |= NRF_WIFI_CMD_AUTHENTICATE_LOCAL_STATE_CHANGE;i
			    ...
			    int ifindex, enum nl80211_iftype mode)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Set mode ifindex %d iftype %d (%s)",
      ...
      wpa_printf(MSG_DEBUG, "nl80211: %s: sending NRF_WIFI_UMAC_CMD_SET_INTERFACE command\n", __func__);
      ret = send_and_recv_msgs_cfg(drv, get_setintf_res_cfg, &arg, chg_vif_cmd, //AJAY
      ...
      if (!(*arg.res))
              return 0;
       wpa_printf(MSG_DEBUG, "nl80211: Failed to set interface %d to mode %d:" //AJAY
...
static void nl80211_frame_wait_cancel(struct i802_bss *bss, u64 cookie)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Cancel TX frame wait: cookie=0x%llx",
		   (long long unsigned int) cookie);
#ifdef CFG_INTERFACE
    //TODO: Currently not supported in  driver
    return 0;
#else
	...
	enum wpa_drv_update_connect_params_mask mask)
{
	...
	enum nl80211_auth_type type;
#ifdef CFG_INTERFACE
	//NL80211_CMD_UPDATE_CONNECT_PARAMS is not supported
	return ret;
#else
...
#ifndef CFG_INTERFACE
	/*Currently this is not supported for UMAC CFG case*/
